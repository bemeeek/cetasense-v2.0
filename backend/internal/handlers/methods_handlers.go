...
func (h *MethodsHandler) DeleteMethod(w http.ResponseWriter, r *http.Request) {
	...
	t0 := time.Now()
	methodID := vars["id"]
	metrics.Step(reqID, "DELETE_METHOD_START", float64(time.Since(t0).Nanoseconds())/1e6)
...
func (h *MethodsHandler) RenameMethod(w http.ResponseWriter, r *http.Request) {
	...
	t0 := time.Now()
	methodID := vars["id"]
	metrics.Step(reqID, "RENAME_METHOD_START", float64(time.Since(t0).Nanoseconds())/1e6)
